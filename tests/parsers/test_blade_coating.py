...
def test_tfsc_batch_parser(monkeypatch):
    ...
    PROCESS_CHECKS = {
        ...
        'sau_geso_1': check_batch,
        'sau_geso_1_1': check_subbatch,
...
def check_batch(m):
    assert m.data.name == 'SAU_GeSo_1'
    assert m.data.lab_id == 'SAU_GeSo_1'
...
def check_blade_coating(m):
    ...
    assert m.data.properties['blade_speed'] == 1 * ureg('mm/s')
    assert m.data.properties['dispensed_volume'] == 4 * ureg('uL')
    assert m.data.properties['blade_substrate_gap'] == 100 * ureg('um')
    assert m.data.properties['blade_size'] == 'free text'
    assert m.data.properties['coating_width'] == 100 * ureg('mm')
    assert m.data.properties['coating_length'] == 200 * ureg('mm')
    assert m.data.properties['dead_length'] == 50 * ureg('mm')
    assert m.data.properties['bed_temperature'] == ureg.Quantity(60, ureg('°C'))
    assert m.data.properties['ink_temperature'] == ureg.Quantity(40, ureg('°C'))
